...
strDateTimeNTP DateTimeNTP;                      // Global DateTime structure, will be refreshed every Second
...
strDateTimeNTP ConvertUnixTimeStamp( unsigned long _tempTimeStamp) {
  strDateTimeNTP _tempDateTime;
  uint8_t year;
...
boolean summerTime(unsigned long _timeStamp ) {
  strDateTimeNTP  _tempDateTime = ConvertUnixTimeStamp(_timeStamp);
...
void ISRsecondTick()
{
  ...
  DateTimeNTP = ConvertUnixTimeStamp(absoluteActualTime);  //  convert to DateTime format
  actualTime = 3600 * DateTimeNTP.hour + 60 * DateTimeNTP.minute + DateTimeNTP.second;
